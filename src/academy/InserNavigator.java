...
public class InserNavigator implements Actionable,CodeInfo, Navigatable {
    ...
    public int run() {
        while (true){
            ...
            switch (result+200){
                case INSERT_STUDENT:
                    insert= new InsertTypeFactory(database).createInsert(INSERT_STUDENT);

                    break;
                case INSERT_TEACHER:
                    insert= new InsertTypeFactory(database).createInsert(INSERT_TEACHER);
